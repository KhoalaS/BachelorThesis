...
The $\texttt{HyperGraph}$ datatype has five fields. Both fields $\texttt{Vertices}$ and $\texttt{Edges}$ are maps with keys of type $\texttt{int32}$ and values of type $\texttt{Vertex}$ and $\texttt{Edge}$ respectively. We chose this set-like datastructure over lists again because of faster access times, but also operations that remove edges/vertices are built-in to the map type. The field $\texttt{edgeCounter}$ is an internal counter used to assign ids to added edges. The field $\texttt{IncMap}$ is a map of maps, essentially storing the hypergraph as a sparse incidence matrix. We will also derive vertex degrees from this map. And at last the $\texttt{AdjCount}$ map, which will associate every vertex $v \in V$ to all vertices adjacent to $v$. Additionally this map will also store the amount of times such a vertex is adjacent to $v$.
...
This way we minimize memory writes by reusing existing data structures we built in step 1, but also save on execution time, since we mutate and iterate the auxiliary data structure at the same time. We can also reuse these algorithms with minimal changes, in the case where we want to only execute a rule once.
...
### F3 Target Low Degree Rule

-   F3 target low degree: Let $v$ be a vertex with degree 2 and let $e$ be an edge that is incident to $x$.If there exists an edge $f$, that is incident to a vertex in $e$ and does not contain $v$, put $f$ into the hitting set.
...
The goal of the F3 target low degree rule is to reduce the degree of a vertex, which preferably has a degree of two. Applying the rule would lower the degree of the vertex to one, making the vertex domination rule applicable. We tested the effectiveness of the F3 target low degree rule on random ER hypergraphs. First we generated 100 random ER hypergraphs with 1000 vertices and approximatly 3000 edges. We ran our algorithm on every graph 10 times each to account for run to run varaince. We collected data for two datasets. For the first dataset we selected a random size three edge when applying the fallback rule. For the second dataset we used the F3 low target degree rule instead. The results can be seen in table \ref{er3_evr3_stats}.
...
\fbox{
    \begin{minipage}[h]{0.75\textwidth}
        ...
        \textbf{Parameter:} A non-negative integer $k$.
...
The LP relaxation of this ILP would allow for $x_i$ to be any value in the intervall $[0,1]$ for all $i \in [n]$. The dual to the problem, the $\textsc{Set Cover}$ problem, can be formulated similarly for the unweighted case,
...
And again, the LP relaxation of this ILP would allow for $x_j$ to be any value in the intervall $[0,1]$ for all $j \in [m]$.

It might not obvious at first that \textsc{Set Cover} is the dual of \textsc{Hitting Set}, but formulating both problems informally gives it away in an understandable way. One problem tries to cover sets with elements, while the over aims at covering elements with sets. For a formal reduction from \textsc{Hitting Set} to \textsc{Set Cover} let $G=(V,E)$ be a hypergraph and $C \subseteq V$. Let $S_v = \left\{ e \in E \mid v \in e \right\}$ for $v\in V$, i.e. the set of edges that are incident to a vertex $v$. Now $C$ is a hitting set if and only if $S = \left\{ e \in S_v \mid v \in C\right\}$ is a set cover. Let us assume that $C$ is a hitting set for $G$, then it holds that,
...
\begin{table}[t]
     ...
     \begin{tabular}{c|rrrrrr}
          \toprule
          algorithm                      & solver & ratio UB  & actual ratio &
          $\textnormal{Opt}^*$           & $|C|$  & solver time                                               \\
          \midrule
          \multirow{3}{4em}{Hitting Set LP (1)} & GLPK   & \multirow{3}{3em}{2.9986}         & 1.5814       & 64452.6941 &
          101928                         & 1 hour                                                             \\
                                         & CLP    &            & 1.7838       & 64452.6992 & 114972 & 50 sec \\
                                         & HiGHS  &            & 1.6709       & 64452.6993 & 107697 & 17 sec \\
          \midrule
          \multirow{3}{4em}{Set Cover LP (2)}   & GLPK   & \multirow{3}{3em}{2.9148} & 1.5944       & 64452.6940 &
          102762                         & 1 hour                                                             \\
                                         & CLP    &            & 1.8068       & 64452.6992 & 116455 & 47 sec \\
                                         & HiGHS  &            & 1.6826       & 64452.6993 & 108447 & 19 sec \\
     \end{tabular}
...
Algorithm (1) slightly outperforms algorithm (2). The LP solution obtained with GLPK yields the smallest hitting set for both algorithms, but also takes the longest to compute at an execution time of about one hour. HiGHS is the fastest solver and produces a hitting set that is around five percent larger than GLPK. The CLP solver is not very well suited for the algorithms on this specific problem instance. Execution time aswell as the resulting hitting set size are worse compared to HiGHS. Compared to our algorithm, both algorithm (1) and (2) yield hitting sets with bigger size.
...
\caption{target low degree edge in F3 rule}
\end{subtable}
